...
public interface TurmaRepository extends PagingAndSortingRepository<Turma, Long> {
	...
	@Query("SELECT t FROM Turma t WHERE t.codigo LIKE %:p% OR t.curso.nome LIKE %:p% OR t.status LIKE %:p% OR t.diasDaTurma LIKE %:p% OR t.periodo LIKE %:p% OR t.instrutor.nome LIKE %:p% OR t.ambiente.nome LIKE %:p% OR t.qtdMatriculas LIKE %:p% OR t.numMaxVagas LIKE %:p%")
